...
//  uint16_t up_edge_cnt=0;
 ...
 extern uint16_t sync_alarm_flag;
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    ...
    if(TIM1 == htim->Instance)
    {
        ...
        switch(Sync_Cnt){
			...
			if(htim->Channel==HAL_TIM_ACTIVE_CHANNEL_1){   //检测信号输入    捕获下降沿，避免定时器2切换开关时产生冲突
				cnt++;
				switch(cnt){
					case 1:
					...
					cnt=0;
				if(duration>=0x04&&duration<=0x06){
