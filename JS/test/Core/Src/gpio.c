...
void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, ACTION1_Pin|RELAY_CTRL_Pin|sync_alarm_Pin|output_led_Pin
                          |system_led_Pin, GPIO_PIN_SET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, EN_R2_Pin|ACTION_Pin|A2_R_Pin|EN_R1_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, A1_R_Pin|A0_R_Pin|A3_R_Pin, GPIO_PIN_RESET);
  ...
  GPIO_InitStruct.Pin = EN_R2_Pin|A2_R_Pin|EN_R1_Pin;
...
/* USER CODE BEGIN 2 */
void Channel_Sel(uint8_t ch){                     //MPCÍ¨µÀÇÐ»»
	switch(ch%8){
		case 0:
...
void Relay_Action_Open(void){     //¼ì²âµ½ÐÅºÅ
...
void Relay_Action_Close(void){   //³õÊ¼×´Ì¬
