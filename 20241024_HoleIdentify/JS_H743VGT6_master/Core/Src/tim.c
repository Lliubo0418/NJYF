...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  if (htim->Instance == TIM6)
  {
    ...
    if (IsMpc1)
    {
      ...
      R1_R2_Channel_Sel(Mpc1_Channel_Num);
      R3_R4_Channel_Sel(Mpc1_Channel_Num);
    ...
    else
    {
      ...
      R1_R2_Channel_Sel(Mpc2_Channel_Num);
      R3_R4_Channel_Sel(Mpc2_Channel_Num);
  ...
  //
  // LXXX:tim7已设置为us级阻塞延时定时器
  if (htim->Instance == TIM7)
  {
    if (TIM7->SR & TIM_SR_UIF)
    {                            // 检查更新中断标志
      TIM7->SR &= ~TIM_SR_UIF;   // 清除更新中断标志
      TIM7->CR1 &= ~TIM_CR1_CEN; // 停止定时器
      BaseType_t xHigherPriorityTaskWoken = pdFALSE;
      xSemaphoreGiveFromISR(xTimerSemaphore, &xHigherPriorityTaskWoken);
      portYIELD_FROM_ISR(xHigherPriorityTaskWoken); // 在中断中切换任务
    }
  }
}
// us级阻塞延时函数
void MicrosecondsDelay(uint32_t us)
{
  // // 清除残留信号量，避免误唤醒
  // xSemaphoreTake(xTimerSemaphore, 0);
  TIM7->CNT = 0;
  TIM7->ARR = us - 1; // 设置自动重装载值
  TIM7->CR1 |= TIM_CR1_CEN;
  //  while (TIM7->CNT < (us-2));
  ////  TIM7->CR1 &= ~TIM_CR1_CEN;
  xSemaphoreTake(xTimerSemaphore, portMAX_DELAY); // 等待信号量，任务挂起
}
/* USER CODE END 1 */
