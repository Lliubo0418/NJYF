...
extern uint8_t g_usb_usart_rx_buffer[USB_USART_REC_LEN];       /* 接收缓冲,最大USART_REC_LEN个字节 */
 ...
 uint16_t sin_data[200], cos_data[200];

uint16_t disposal_cnt = 60000, read_cnt = 40000;
...
void Prepare_1024Byte_Response(uint8_t *buffer, uint8_t flag, uint16_t start_pos)
{
    ...
    Get_ADC_Data(flag, start_pos, sin_data, cos_data, 200);

		for (uint16_t i = 0; i < 200; i++) {
				uint16_t sin_val = sin_data[i];
				uint16_t cos_val = cos_data[i];

    // 交替写入 sin、cos 两个字节（高位在前，即大端）
			adc_params->adc_data[i * 4]     = (sin_val >> 8) & 0xFF; // sin high byte
			adc_params->adc_data[i * 4 + 1] = sin_val & 0xFF;        // sin low byte
			adc_params->adc_data[i * 4 + 2] = (cos_val >> 8) & 0xFF; // cos high byte
			adc_params->adc_data[i * 4 + 3] = cos_val & 0xFF;        // cos low byte
}

    // === 4. 清空 checksum 字节，避免参与校验计算 ===
    adc_params->checksum = 0;

    // === 5. 将所有 uint16_t 字段转换为大端（字节3~848） ===
...
{
    Prepare_1024Byte_Response(g_usb_tx_adc_buffer, ws_ds_flag, start_pos);

    udbd_cdc_tx_data(g_usb_tx_adc_buffer,1024);
    delay_us(2000);
...
void Get_ADC_Data(uint8_t flag, uint16_t start_pos, uint16_t *sin_data, uint16_t *cos_data, uint16_t count)
{
    ...
    for (uint16_t i = 0; i < count; i++) {
		...
		if(flag	== DEVICE_SIDE_WS){
			//读取SRAM中WS的数据
			sram_side_read(0, DEVICE_SIDE_WS);   //sin
			sram_side_read(1, DEVICE_SIDE_WS);   //cos
		
		}
		else{
			//读取SRAM中DS的数据
			sram_side_read(0, DEVICE_SIDE_DS);   //sin
			sram_side_read(1, DEVICE_SIDE_DS);   //cos		
