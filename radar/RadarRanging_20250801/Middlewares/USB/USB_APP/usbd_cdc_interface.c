...
uint8_t waiting_for_ack = 0; // 标志位，1表示等待 0x06，0表示不等待
extern uint16_t  disposal_cnt , read_cnt ;
    ...
    if (g_usb_usart_rx_sta >= 5) {
        ...
        if (g_usb_usart_rx_buffer[0] == 0x15) {
            ...
            if (g_usb_usart_rx_buffer[1] == 0x80) {
                ...
                /* 读取 SRAM */
								disposal_cnt = g_usb_usart_rx_buffer[2]<<8|g_usb_usart_rx_buffer[3];
								read_cnt   = g_usb_usart_rx_buffer[4]<<8|g_usb_usart_rx_buffer[5];
