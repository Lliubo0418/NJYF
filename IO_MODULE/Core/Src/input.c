...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)//更新中断（溢出）发生时执行
{
	...
	{
		if(MCU_F3_CAPTURE_STA&0X40)				//已经捕获到高电平了
		{
			if((MCU_F3_CAPTURE_STA&0X3F)==0X3F)	//高电平太长了
			{
				MCU_F3_CAPTURE_STA|=0X80;			//标记成功捕获了一次
				MCU_F3_CAPTURE_VAL=0XFFFF;
					}else MCU_F3_CAPTURE_STA++;
				}	 
			}
		}
	}
	...
	{
		if(MCU_F2_CAPTURE_STA&0X40)				//已经捕获到高电平了
		{
			if((MCU_F2_CAPTURE_STA&0X3F)==0X3F)	//高电平太长了
			{
				MCU_F2_CAPTURE_STA|=0X80;			//标记成功捕获了一次
				MCU_F2_CAPTURE_VAL=0XFFFF;
					}else MCU_F2_CAPTURE_STA++;
				}	 
			}
		}
	}
