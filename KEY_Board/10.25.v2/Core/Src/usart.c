...
__IO uint8_t KeyValue=0xFF, KeyValueKeep=0xFF;              //修复初始化为0时，上电第一次按下1键造成的冲突
...
void USB_Key_Update(uint8_t index)
{
    ...
    KeyCmd[5] = (index == 2 || index == 20) ? 0x02 : 0x00;  // 特殊键值处理
    KeyCmd[6] = 0x00;
...
void KeyModbusBy485(uint8_t index){
    	// 等待发送完成标志位清零
    while (Uart1TxDoneFlag == 1);
    Uart1TxDoneFlag = 1;

    // 初始化指令数据
    ModbusCmd[0] = 0x01;          //设备地址
    ModbusCmd[1] = 0x05;          //写线圈
    ModbusCmd[2] = 0x00;          
    ModbusCmd[3] = index;         //寄存器地址
    ModbusCmd[4] = 0x00;
    ModbusCmd[5] = KeyAsciiIndex[index];          //写入数据
    // 计算 CRC 校验
    uint16_t crc = CalculateCRC16(ModbusCmd, 6);

    // 将 CRC 结果存储到 ModbusCmd 的最后两个字节
    ModbusCmd[6] = crc & 0xFF;         // CRC 低字节
    ModbusCmd[7] = (crc >> 8) & 0xFF;  // CRC 高字节
