...
void StartFreq_ICTask(void const * argument)
{
  ...
  for (;;)
  {
    if (power_on_init)
    {
      DATA = (float)(0.397 * DAC_COEFFICIENT); // 4mA
      SPI_DAC8560_Write(DAC_DEFAULT_MODE, DATA);
			SPI_DAC8560_Write(DAC_DEFAULT_MODE, DATA);
      power_on_init = 0;
    }
    ...
    if (ret == pdPASS)
    {
      if (Frequency_old != Frequency)
      {
        ...
        else if (Frequency == 0)
        {
          ...
          DATA = (float)(0.396 * DAC_COEFFICIENT); // 4mA
          SPI_DAC8560_Write(DAC_DEFAULT_MODE, DATA);
          Frequency_old = Frequency;
        }
        else if (Frequency > Frequency_Input_Max)
        {
          ...
          DATA = (float)(1.996 * DAC_COEFFICIENT); // 20mA
          SPI_DAC8560_Write(DAC_DEFAULT_MODE, DATA);
        }
      }
    }
...
void PWM_SetFrequencyDutyCycle(float frequency)
{
//  HAL_TIM_PWM_Stop(&htim3, TIM_CHANNEL_4); // 关闭TIM4通道4的PWM
  // 计算定时器的时钟频率
  ...
  htim3.Instance->ARR = period; // 设置自动重装载寄存器（周期）

  // 设置PWM占空比
  htim3.Instance->CCR4 = (uint32_t)round(((float)(period + 1) / 2)); // 设置比较寄存器（占空比）

  // 启动PWM
//  HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_4); // 启动TIM4通道4的PWM
}
